...
suite(`AddUserValidation`, () => {
  setup(() => {
    ...
    container.req = req;
    container.res = res;
    container.loginUserValidationMiddleware = loginUserValidationMiddleware;
  });

  test(`Should error for add new user if send empty body`, async () => {
    container.req.body = {};

    const badCall = container.loginUserValidationMiddleware.act();
  ...
  test(`Should successfully for add new user`, async () => {
    ...
    await container.loginUserValidationMiddleware.act();
  });
});
